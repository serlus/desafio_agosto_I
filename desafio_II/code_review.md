...
Querido João, senti falta de uma docstring explicando um pouco 
sobre a funcionalidade que busca com o projeto. Realizando uma 
revisão com flake8 reparei:
 - importes não estão sendo utilizados 
   ('traceback', 'timedelta' e 'timezone'), vocês tinha intenção 
   de utilizalos em algum retorno?
 - uma boa prática tb seria não colocar mais de um importe em uma 
    única linha
...
Acredito que pode ajudar separa o main em um modulo separado
```python
def main(argv):
    greetings()
